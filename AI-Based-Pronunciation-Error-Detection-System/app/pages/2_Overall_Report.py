...
word_data = word_data.set_index("Word")

chart_choice = st.selectbox("Choose chart presentation:", ["Progress", "Bar", "Pie"])

if chart_choice == "Bar":
    st.markdown("**Bar chart (scores by word)**")
    st.bar_chart(word_data)

elif chart_choice == "Pie":
    st.markdown("**Pie chart (score category distribution)**")
    # categories for pie
    categories = {
        "Excellent (>=80)": sum(1 for s in word_data["Score"] if s >= 80),
        "Good (50-79)": sum(1 for s in word_data["Score"] if 50 <= s < 80),
        "Needs work (<50)": sum(1 for s in word_data["Score"] if s < 50),
    }
    import matplotlib.pyplot as plt
    fig, ax = plt.subplots(figsize=(5, 4))
    ax.pie(categories.values(), labels=categories.keys(), autopct="%1.0f%%", startangle=90,
           colors=["#27ae60", "#f39c12", "#e74c3c"])
    ax.axis("equal")
    st.pyplot(fig)

else:
    st.markdown("**Progress bars (per-word)**")
    for wr in wrs:
...
st.divider()
# ── Progress by category (new chart block) ───────────────────────────────────
st.subheader("📊 Progress by category")
if wrs:
    clarity = round(sum(wr.score for wr in wrs)/len(wrs))
    fluency = round(sum(100 - abs(100 - wr.score) for wr in wrs)/len(wrs))
    accuracy = round(sum(wr.score for wr in wrs)/len(wrs))
    speed = max(40, min(100, round(sum(wr.score for wr in wrs)/len(wrs) - 10)))
else:
    clarity, fluency, accuracy, speed = 80, 75, 82, 68
...
for name, value in category_progress.items():
    st.markdown(f"**{name}: {value}%**")
    st.progress(value / 100)

# --- optional chart controls for summary
chart_type = st.selectbox("Choose visual for category breakdown:", ["Bar", "Pie"])

import pandas as pd
cat_df = pd.DataFrame({"Percent": list(category_progress.values())}, index=list(category_progress.keys()))
...
if chart_type == "Bar":
    ...
    import matplotlib.pyplot as plt
    fig, ax = plt.subplots(figsize=(5, 4))
    ax.pie(cat_df["Percent"], labels=cat_df.index, autopct="%1.0f%%", startangle=140,
           colors=["#4f8bc9", "#6ec9a3", "#f39c12", "#8e44ad"])
    ax.axis("equal")
    st.pyplot(fig)
