...
_model_size       = os.getenv("WHISPER_MODEL", "small").split("#", 1)[0].strip()
...
def _preprocess_for_asr(audio_path: str | Path) -> str:
    ...
    try:
        ...
        from infrastructure.audio_processing import trim_silence
        audio = trim_silence(audio, threshold=0.01, min_silence_duration=0.1)
        ...
        logger.debug("ASR preprocessing failed for %s: %s", audio_path, exc)
...
def _timestamps_faster_whisper(
    ...
    segments, _ = model.transcribe(
        ...
        beam_size=5,
        best_of=1,
        initial_prompt="The user is speaking English sentences clearly.",
    )
